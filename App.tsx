...
const App: React.FC = () => {
    ...
    Array.from(files).forEach((file: File) => {
        ...
        reader.onload = (event) => {
          ...
          img.onload = () => {
            ...
            setPhotos((prev) => [newItem, ...prev].slice(0, 30));
          };
        };
  ...
  const toggleLetterFocus = () => {
    if (isLetterFocused) {
        setIsLetterFocused(false);
        // If we close the letter, we return to whatever the primary interaction mode was
        // or default back to TREE if it was in FOCUS mode.
        if (interactionMode === InteractionMode.FOCUS) {
